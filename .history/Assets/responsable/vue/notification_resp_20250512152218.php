...
if (!isset($_SESSION['is_responsable']) || $_SESSION['is_responsable'] != true) {
...
$sql = "SELECT * FROM notifications_responsable WHERE Matricule_resp = :user_id ORDER BY date_notif DESC";
...
WHERE Matricule_responsable = :user_id AND Statut_notif = 'non lu' ";
...
<script>
  ...
  function showNotificationDetails(id, genre, message, date, type) {
    ...
    document.getElementById('deleteLink').href = '../traitement/delete_responsable_notification.php?id=' + id;
  ...
  function markAsRead(id) {
    ...
    fetch('../traitement/mark_responsable_notification_read.php?id=' + id)
      .then(response => {
        if (!response.ok) {

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (empty($tache) || empty($date_tache) || $duree_tache <= 0) {
            $_SESSION['error'] = "Veuillez remplir tous les champs obligatoires correctement.";
            header('Location: ../employe/timesheet.php');
            exit();
        }

        // Insérer la tâche dans la base de données
        $sql = "INSERT INTO timesheet (matricule_emp, tache, date_tache, duree_tache, client, description_tache, note) 
                VALUES (:matricule_emp, :tache, :date_tache, :duree_tache, :client, :description_tache, :note)";
        ...
        header('Location: ../employe/mes_timesheet.php');
        exit();

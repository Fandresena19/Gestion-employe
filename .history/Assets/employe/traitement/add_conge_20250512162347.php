...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result) {
        ...
        $notif = "INSERT INTO notifications_resp (matricule_emp, Genre_notif, Message_resp, type, date_notif_resp, statut_notif_resp)
        VALUES ( :matricule_emp, 'Nouveau' , :message, 'Congé', :date_notif_resp, :Statut_notif_resp)";
        $stmt = $bdd->prepare($notif);
        $result_notif = $stmt->execute([
            'matricule_emp' => $matricule_emp,
            'message' => "L'employé " .$matricule_emp['nom_emp']." ".$matricule_emp['prenom_emp']. " a soumis une demande de congé de $dureeAbsenceJ jours $dureeAbsenceH Heures",

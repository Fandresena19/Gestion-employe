...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($result) {
    ...
    $emp = $bdd->query("SELECT * FROM employer_login WHERE matricule_emp = '$matricule_emp'");

    $notif = "INSERT INTO notification_responsable 
    (matricule_emp, Genre_notif, Message_resp, type, date_notif_resp, statut_notif_resp)
    VALUES (:matricule_emp, 'Nouvelle permission' , :message, 'Permission', :date_notif_resp, :Statut_notif_resp)";
    $stmt = $bdd->prepare($notif);
    $result_notif = $stmt->execute([
      'matricule_emo' => $matricule_emp,
      'message' => "L'employé " .$emp['nom_emp']. " " .$emp['prenom_emp']. "a soumis une nouvelle demande de permission 
      de " .$jourPermission. " jours et " .$heurePermission. " heures pour le motif ",

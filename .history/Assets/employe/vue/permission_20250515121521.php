...
$result = $bdd->query("SELECT id_permission FROM permission ORDER BY id_permission DESC LIMIT 1");

// Vérifier si un matricule existe déjà
$last_id = ($result->rowCount() > 0) ? intval($result->fetch()['id_permission']) + 1 : 1;
?>
...
<div class="contenu">
  ...
  <form action="../traitement/add_permission.php" method="post">
    ...
    <div class="form-group">
      ...
      <input name="ref" type="text" class="form-control" value="<?= "Permission N°: ".$last_id; ?>" readonly />
    </div>

...
<?php
// Ajouter ce code avant de traiter le formulaire dans add_conge.php
...
// Après insertion dans la table conge
$matricule_emp = $_SESSION['Matricule_emp'];
$sql = "SELECT prenom, nom FROM employes WHERE Matricule_emp = ?";
$stmt = $bdd->prepare($sql);
$stmt->execute([$matricule_emp]);
$employe = $stmt->fetch();

$nom_prenom = $employe['prenom'] . ' ' . $employe['nom'];
$message = "Nouvelle demande de congé de $nom_prenom";
$type = "Nouveau";
$date_notif = date("Y-m-d H:i:s");

// Récupérer tous les responsables
$sql = "SELECT Matricule_emp FROM employes WHERE is_responsable = 1";
$responsables = $bdd->query($sql)->fetchAll(PDO::FETCH_ASSOC);

foreach ($responsables as $resp) {
  $insertNotif = $bdd->prepare("INSERT INTO notifications_responsable 
        (Matricule_resp, Genre, Message, Type, date_notif_resp, Statut_notif_resp)
        VALUES (?, 'Congé', ?, ?, ?, 'non lu')");
  $insertNotif->execute([
    $resp['Matricule_emp'],
    $message,

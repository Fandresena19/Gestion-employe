...
$result = $bdd->query("SELECT matricule_emp, matricule FROM employer_login ORDER BY matricule_emp DESC LIMIT 1");

// Vérifier si un matricule existe déjà
$last_id = ($result->rowCount() > 0) ? intval($result->fetch()['matricule_emp']) + 1 : 1;

$result = $bdd->query("SELECT matricule FROM employer_login ORDER BY matricule_emp DESC LIMIT 1");

if ($result->rowCount() > 0) {
    $lastMatricule = $result->fetch()['matricule']; // Ex: EMP003
    $num = intval(substr($lastMatricule, 3));       // Extrait 003 → 3
    $newMatricule = 'EMP' . str_pad($num + 1, 3, '0', STR_PAD_LEFT); // EMP004
} else {
    $newMatricule = 'EMP001';
}
